...
from model.QMVOS_trainer import QMVOS_Trainer
from dataset.static_dataset import StaticTransformDataset
from dataset.vos_dataset import VOSDataset

from util.logger import TensorboardLogger
from util.configuration_QMVOS import Configuration
...
for si, stage in enumerate(stages_to_perform):
    ...
    if local_rank == 0:
        ...
        model = QMVOS_Trainer(config, logger=logger, 
        ...
        model = QMVOS_Trainer(config, local_rank=local_rank, world_size=world_size).train()
    ...
    if stage == '0':
        ...
        davis_root = path.join(path.expanduser(config['davis_root']))
        ...
        while total_iter < config['iterations'] + config['finetune']:
            ...
            for data in train_loader:
                ...
                if config['finetune'] > 0 and not finetuning and total_iter >= config['iterations']:
                    ...
                    finetuning = True
                    model.save_network_interval = 500 # 1000, 当batch-size变大后降为原来1/4
                    break
