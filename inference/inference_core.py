from inference.memory_manager import *
...
class InferenceCore_QMVOS:
    def __init__(self, network: QMVOS_Network, config):
        self.config = config
        self.network = network
        self.mem_every = config['mem_every']
        self.deep_update_every = config['deep_update_every']
    ...
    def step(self, image, query_embed_object, num_objects, mask=None, valid_labels=None, end=False):
        # image: 3*H*W
        # mask: num_objects*H*W or None
        self.curr_ti += 1
        image, self.pad = pad_divide_by(image, 16)
        ...
        f16_query, key, shrinkage, selection, f16, f8, f4 = self.network.encode_key(image, 
                                                    need_ek=(self.enable_long_term or need_segment), 
                                                    need_sk=is_mem_frame)
        multi_scale_features = (f16, f8, f4)
        ...
        if need_segment:
            ...
            memory_readout = self.memory.match_memory(key, selection).unsqueeze(0)
            hidden, _,_, pred_prob_with_bg = self.network.segment_QCIM(query_embed_object,[num_objects],multi_scale_features, memory_readout, 
                                    self.memory.get_hidden(), h_out=is_normal_update, strip_bg=False)
            masks_w_bg=prob2mask(pred_prob_with_bg)
            query_embed_object=self.network.init_query_SIM((f16_query.unsqueeze(1),f8.unsqueeze(1)),masks_w_bg.unsqueeze(1),[num_objects])
            # remove batch dim
            pred_prob_with_bg = pred_prob_with_bg[0]
            pred_prob_no_bg = pred_prob_with_bg[1:]
            if is_normal_update:
        ...
        if mask is not None:
            mask, _ = pad_divide_by(mask, 16)
            num_objects, _, _=mask.shape
            first_frame_gt=mask.reshape(1,1,num_objects,*mask.shape[-2:])
            query_embed_object=self.network.init_query_SIM((f16_query.unsqueeze(1),f8.unsqueeze(1)),first_frame_gt,[num_objects])
        ...
        return unpad(pred_prob_with_bg, self.pad),query_embed_object
