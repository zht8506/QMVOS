...
from inference.kv_memory_store import *
...
class MemoryManager_query:
    ...
    def __init__(self, config):
        ...
        self.work_mem = KeyValueQueryMemoryStore(count_usage=self.enable_long_term)
        if self.enable_long_term:
            self.long_mem = KeyValueQueryMemoryStore(count_usage=self.enable_long_term_usage)
    ...
    def get_query(self):
        return self.work_mem.get_query()
    
    def get_mem_value(self):
        return self.work_mem.value
    ...
    def match_memory(self, query_key, selection):
        ...
        return all_readout_mem.view(all_readout_mem.shape[0], self.CV, h, w),affinity

    def add_memory(self, key, shrinkage, value,query_embed, objects, selection=None):
        # key: 1*C*H*W
        # value: 1*num_objects*C*H*W
        # objects contain a list of object indices
        if self.H is None or self.reset_config:
        ...
        self.work_mem.add(key, value, shrinkage, selection, objects,query_embed)
    ...
    def add_query(self, query_embed):

        self.work_mem.add_query(query_embed)
