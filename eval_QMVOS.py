...
from inference.inference_core import InferenceCore_QMVOS
...
"""
sub_save_dir="QCIM"
save_name="QCIM_65000_step"

parser = ArgumentParser()
parser.add_argument('--model', default='./xxxx.pth')

# Data options
parser.add_argument('--d16_path', default='/xxxxx/DAVIS')
parser.add_argument('--d17_path', default='/xxxxx/DAVIS_2017_trainval')
parser.add_argument('--y18_path', default='/xxxxx/YouTube2018')
parser.add_argument('--y19_path', default='/xxxxx/YouTube')
...
if args.output is None:
    args.output ='./output/'+sub_save_dir+f'/{args.dataset}_{args.split}'+"_"+save_name
...
if is_youtube:
    ...
    if args.dataset == 'D16':
        if args.split == 'val':
            ...
            meta_dataset = DAVISTestDataset(args.d16_path, imset='../../DAVIS_2017_trainval/ImageSets/2016/val.txt', size=args.size)
        else:
            raise NotImplementedError
        palette = None
    elif args.dataset == 'D17':
        if args.split == 'val':
            meta_dataset = DAVISTestDataset(path.join(args.d17_path), imset='2017/val.txt', size=args.size)
        elif args.split == 'test':
...
# Load our checkpoint
network = QMVOS_Network(config, args.model).cuda().eval()
...
for vid_reader in progressbar(meta_loader, max_value=len(meta_dataset), redirect_stdout=True):
    ...
    mapper = MaskMapper()
    processor = InferenceCore_QMVOS(network, config=config)
    ...
    for ti, data in enumerate(loader):
        with torch.cuda.amp.autocast(enabled=not args.benchmark):
            ...
            prob, query_embed_object = processor.step(rgb, query_embed_object,num_objects, msk, labels, end=(ti==vid_length-1))
