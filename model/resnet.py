...
def load_weights_add_extra_dim_MIM(target, source_state, extra_dim=1):
    new_dict = OrderedDict()
    ...
    for k1, v1 in target.state_dict().items():
        if not 'num_batches_tracked' in k1:
            if k1 in source_state:
                tar_v = source_state[k1]
                if "running" not in k1:
                    tar_v.requires_grad_(True)
                if v1.shape != tar_v.shape:
                    # Init the new segmentation channel with zeros
                    # print(v1.shape, tar_v.shape)
                    c, _, w, h = v1.shape
...
def load_state_dict():
    cached_file = "/home/zhouhantao/XMem-Newquery/pretrain/resnet50_1kpretrained_timm_style.pth"
    return torch.load(cached_file, map_location=None)

def resnet50_MIMpre(pretrained=True, extra_dim=0):
    model = ResNet(Bottleneck, [3, 4, 6, 3], extra_dim)
    if pretrained:
        load_weights_add_extra_dim_MIM(model, load_state_dict(), extra_dim)
    return model
