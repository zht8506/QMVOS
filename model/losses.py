    ...
    def compute(self, data, num_objects, it):
        ...
        for ti in range(1, t):
            for bi in range(b):
                ...
                loss_cross, p_cross = self.bce(data[f'logits_cross_{ti}'][bi:bi+1, :num_objects[bi]+1], data['cls_gt'][bi:bi+1,ti,0], it)
                losses['p_cross'] += p_cross / b / (t-1)
                losses[f'ce_loss_cross_{ti}'] += loss_cross / b

            losses['total_loss'] += losses['ce_loss_%d'%ti]
            losses[f'dice_loss_{ti}'] = dice_loss(data[f'masks_{ti}'], data['cls_gt'][:,ti,0])
            losses['total_loss'] += losses[f'dice_loss_{ti}']

            losses['total_loss'] += losses['ce_loss_cross_%d'%ti]
            losses[f'dice_loss_cross_{ti}'] = dice_loss(data[f'masks_cross_{ti}'], data['cls_gt'][:,ti,0])
            losses['total_loss'] += losses[f'dice_loss_cross_{ti}']
