...
from model.network import QMVOS_Network
from model.losses import LossComputer
from util.log_integrator import Integrator
from util.image_saver import pool_pairs
from model.modules import prob2mask
...
class QMVOS_Trainer:
    def __init__(self, config, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        self.QMVOS = nn.parallel.DistributedDataParallel(
            QMVOS_Network(config).cuda(), 
            device_ids=[local_rank], output_device=local_rank, broadcast_buffers=False,find_unused_parameters=False)
        ...
        if logger is not None:
            ...
            self.logger.log_string('model_size', str(sum([param.nelement() for param in self.QMVOS.parameters()])))
        self.train_integrator = Integrator(self.logger, distributed=True, local_rank=local_rank, world_size=world_size)
        self.loss_computer = LossComputer(config)

        self.train()
        self.optimizer = optim.AdamW(filter(
            lambda p: p.requires_grad, self.QMVOS.parameters()), lr=config['lr'], weight_decay=config['weight_decay'])
    ...
    def do_pass(self, data, it=0):
        ...
        out = {}
        frames = data['rgb'] # [batch-size,num-frames,channels,H,W]
        ...
        with torch.cuda.amp.autocast(enabled=self.config['amp']):
            ...
            f16_query, key, shrinkage, selection, f16, f8, f4 = self.QMVOS('encode_key', frames)

            filler_one = torch.zeros(1, dtype=torch.int64)
            hidden = torch.zeros((b, num_objects, self.config['hidden_dim'], *key.shape[-2:]))
            v16, hidden = self.QMVOS('encode_value', frames[:,0], f16[:,0], hidden, first_frame_gt[:,0])
            values = v16.unsqueeze(3) # add the time dimension

            # init query using Scale-aware Interaction Module (SIM)
            query_embed_object = self.QMVOS('init_query_SIM',(f16_query[:,0], f8[:,0]), first_frame_gt, num_filled_objects)

            # train the model from the second frame
            for ti in range(1, self.num_frames):
                ...
                # read memory
                memory_readout = self.QMVOS('read_memory', key[:,:,ti], selection[:,:,ti] if selection is not None else None, 
                                        ref_keys, ref_shrinkage, ref_values)
                # Segment frame ti
                hidden, logits, masks, masks_w_bg = self.QMVOS('segment_QCIM', query_embed_object, num_filled_objects, (f16[:,ti], f8[:,ti], f4[:,ti]), memory_readout, 
                        hidden, selector, h_out=(ti < (self.num_frames-1)))
                
                # get mask of previous prediction
                masks_w_bg=prob2mask(masks_w_bg)
                
                query_embed_object=self.QMVOS('init_query_SIM',(f16_query[:,ti],f8[:,ti]), masks_w_bg.unsqueeze(1), num_filled_objects)

                # No need to encode the last frame
                if ti < (self.num_frames-1):
                    is_deep_update = np.random.rand() < self.deep_update_prob
                    v16, hidden = self.QMVOS('encode_value', frames[:,ti], f16[:,ti], hidden, masks, is_deep_update=is_deep_update)
    ...
    def save_network(self, it):
        ...
        torch.save(self.QMVOS.module.state_dict(), model_path)
    ...
    def save_checkpoint(self, it):
        ...
        checkpoint = { 
            'it': it,
            'network': self.QMVOS.module.state_dict(),
    ...
    def load_checkpoint(self, path):
        ...
        map_location = 'cuda:%d' % self.local_rank
        self.QMVOS.module.load_state_dict(network,strict=False)
    ...
    def load_network_in_memory(self, src_dict):
        self.QMVOS.module.load_weights(src_dict)
