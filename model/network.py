...
class QMVOS_Network(nn.Module):
    def __init__(self, config, model_path=None, map_location=None):
        ...
        self.key_encoder = KeyEncoder()
        self.simple_upsample_block=SimpleUpsampleBlock(256,512,256,256)
        self.value_encoder = ValueEncoder(self.value_dim, self.hidden_dim, self.single_object)
        ...
        self.query_SelfAtt_FFN = SelfAtt_FFN(d_model=256)

        self.decoder = Decoder_QCIM(self.value_dim, self.hidden_dim)
    ...
    def encode_key(self, frame, need_sk=True, need_ek=True): 
        ...
        if need_reshape:
            ...
            # B*T*C*H*W
            f16_query=f16_query.view(b, t, *f16_query.shape[-3:]) 
            f16 = f16.view(b, t, *f16.shape[-3:])
            f8 = f8.view(b, t, *f8.shape[-3:])
            f4 = f4.view(b, t, *f4.shape[-3:])

        return f16_query, key, shrinkage, selection, f16, f8, f4
    ...
    def init_query_SIM(self, features, mask_pre_frame, num_filled_objects):
        f16, f8 = features
        bs, _, num_objects, H, W = mask_pre_frame.shape

        #----------------------------Scale-aware Initialization-------------------------#
        # multi-scale feature fusion
        supp_feature = self.simple_upsample_block(f16, f8)

        mask_pre_frame = mask_pre_frame.type_as(f16)
        # [bs, 1, num_obj, H, W] -> [bs, num_obj, 1, H, W] -> [bs*num_obj, 1, H, W]
        mask_pre_frame = mask_pre_frame.transpose(1,2).contiguous().view(bs*num_objects, -1, H, W)
        # interpolate to given size
        mask_pre_frame_map = F.interpolate(mask_pre_frame, size=supp_feature.shape[-2:],  mode='nearest')
        mask_pre_frame_map = mask_pre_frame_map.view(bs, num_objects, -1, *supp_feature.shape[-2:])
        
        # init query using mask
        supp_feature = supp_feature[:, 0].unsqueeze(1).expand(-1,num_objects,-1,-1,-1)
        query_embed = Weighted_GAP(supp_feature, mask_pre_frame_map)

        #----------------------------Multi-object Interaction-------------------------#
        # generate mask for self-attention
        maskSelf = torch.zeros(bs, num_objects, num_objects, device=supp_feature.device)
        ...
        # self-attention + ffn
        query_embed=self.query_SelfAtt_FFN(tgt=query_embed, query_pos=None, attn_mask=maskSelf)
    ...
    def segment_QCIM(self, query_embed,num_filled_objects,multi_scale_features, memory_readout,
                    hidden_state, selector=None, h_out=True, strip_bg=True): 

        hidden_state, logits = self.decoder(query_embed,
                                            num_filled_objects,
        ...
        logits, prob = aggregate(prob, dim=1, return_logits=True) # prob is 01 mask
        prob_w_bg=prob
        if strip_bg:
            # Strip away the background
            prob = prob[:, 1:]

        return hidden_state, logits, prob, prob_w_bg
    ...
    def forward(self, mode, *args, **kwargs):
        if mode == 'encode_key':
            ...
            return self.cross_attention(*args, **kwargs)
        elif mode == 'init_query':
            return self.init_query(*args, **kwargs)
        elif mode == 'read_memory':
            return self.read_memory(*args, **kwargs)
        elif mode == 'segment':
            return self.segment(*args, **kwargs)
